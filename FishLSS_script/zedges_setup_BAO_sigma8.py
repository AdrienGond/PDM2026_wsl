...
default_cosmo = DESI_fid


def make_forecast(cosmo,survey_filebase,nbins=3,fsky=0.5,zedges=None):
    ...
    if zedges is not None:
        exp = experiment(zedges=zedges,fsky=fsky,b=b,n=n)
    #
    if typ == 'setup':
        setup = True
    else:
...
def do_task(sfb,typ,nbins,fsky,zedges=None):
    ...
    #    forecast = make_forecast(cosmo,sfb,nbins,fsky)
       forecast = make_forecast(cosmo,sfb,nbins,fsky,zedges=zedges)
       #
    elif typ == 'rec':
       params = {'output': 'mPk'}
       for k in default_cosmo.keys():
...
if __name__=="__main__":
    if len(sys.argv)!=4:
        outstr = "Usage: "+sys.argv[0]+" <survey-filename> <type> <config-file>"
        raise RuntimeError(outstr)

    sfb = sys.argv[1]
    typ = sys.argv[2]
    cfg_file = sys.argv[3]
    if typ not in ("setup", "rec", "fs", "lens"):
        raise ValueError(f"Unknown run type '{typ}'")
    with open(cfg_file,"r") as fp:
        cfg = yaml.safe_load(fp)

    nbins  = cfg.get("nbins", 3)
    fsky   = cfg.get("fsky", 0.5)
    zedges = cfg.get("zedges", None)
    if zedges is not None:
        zedges = np.array(zedges)
        
    do_task(sfb, typ, nbins, fsky, zedges)