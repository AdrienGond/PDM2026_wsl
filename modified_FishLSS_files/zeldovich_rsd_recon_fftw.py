...
class Zeldovich_Recon:
    ...
    def make_psstable(self, f, nu, D = 1, ks = None, kmin = 1e-3, kmax = 3, nk = 100, method='RecSym'):
        ...
        if method == 'RecSym':
            for foo in range(N):
                integ = self.pss_integrals(kv[foo])
                if integ.ndim == 0:
                    self.pktable_ss[foo, 1] = integ
                elif integ.ndim == 1:
                    self.pktable_ss[foo, 1] = integ[0]
                else:
                    raise ValueError("Unexpected dimension of pss_integrals output : {}".format(integ.ndim))
