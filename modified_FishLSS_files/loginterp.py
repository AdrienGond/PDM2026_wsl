...
def derivative(func, x0, dx=1.0, order=9):
    ...
    try:
        ...
        address_log = '/home/adrien/miniconda3/envs/fishlss/lib/python3.11/site-packages/FishLSS/bao_recon/log.npy'

        if os.path.exists(address_log):
            log = np.load(address_log, allow_pickle=True).item()
        else:
            log = {}

        log['n_tot'] = log.get('n_tot', 0) + 1

        if res["status"] != 0:
            log.setdefault('err_rel_value', []).append(res.error / res.df)
            log.setdefault('err_type', []).append(res["status"])
            log['n_err'] = log.get('n_err', 0) + 1

        np.save(address_log, log)

        if (res["status"] != 0) and (res.error/res.df > 1e-5):
            raise ValueError("failed numeric derivative status={}".format(res["status"]))
        return res["df"]
    
    # res = _derivative(func, x0, initial_step=dx, order=order)
    # if res["status"] != 0:
    #     raise ValueError("failed numeric derivative status={}".format(res["status"]) + " rel. error = {}".format(res.error/res.df))
    # return res["df"]
